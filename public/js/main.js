$(() => {
    ...
    $('#btnSend').click(function(e) { 
        ...
        if (chatmsg.username && chatmsg.message && chatmsg.sendDate) {
            postChat(chatmsg);
            socket.emit('messageing', chatmsg);
        }
        $('#txtMessage').val('');
    });

    socket.on('messageing', function(chatmsg) {
        const div = document.createElement('div');
        const header = document.createElement('h6');
        header.innerText = chatmsg.username;
        header.innerHTML += `<span>: ${moment(chatmsg.sendDate).format('LT')}</span>`;
        div.appendChild(header);
        const para = document.createElement('p');
...
function addChatLog(chatmsg) {
    ...
    const header = document.createElement('h6');
    header.innerText = chatmsg.username;
    header.innerHTML += `<span>: ${moment(chatmsg.sendDate).format('DD-MM-YYYY HH:mm:ss')}</span>`;
    div.appendChild(header);
    const para = document.createElement('p');
    para.innerText = chatmsg.message;
    div.appendChild(para);
